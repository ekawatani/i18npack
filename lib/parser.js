...
function Parser(options) {
  ...
  this.languagesLowerCased = _.clone(this.languages).map(function(l) { return l.toLowerCase(); });
  this.schemaDir = options.schemaDir;
  this.delimiter = options.delimiter;
  this.strict = options.strict;
  this.allowEmptyTranslations = options.allowEmptyTranslations;
  this.langIndex = 0;
  ...
  var i18npackTypes = {
    ...
    '!struct mapping': function(value) {
      return nativeTypes.struct(value, options.schemaDir, options.jsonValidatorOptions);
    },
...
Parser.prototype.parse = function(stringData, targetLang) {
  this.langIndex = this.languagesLowerCased.indexOf(targetLang == null ? targetLang : targetLang.toLowerCase());
...
Parser.prototype.process = function(objData) {
  ...
  return traverse(objData).forEach(function(node) {
    if (this.isLeaf) {
      if (!self.allowEmptyTranslations && !!!node) {
        this.delete();
        return;
      }
